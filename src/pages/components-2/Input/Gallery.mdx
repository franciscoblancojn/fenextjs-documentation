...
<Iframe src="https://fenextjs-component-storybook.vercel.app/iframe.html?args=&id=component-input-gallery--index&viewMode=story" />
...
Para ver el storybook del componente lo puede hacer con este [link](https://fenextjs-component-storybook.vercel.app/?path=/story/component-input-gallery--index)
...
```tsx copy
<InputGallery onChange={(items) => { console.log(items); }} />
```

- Galería de imágenes con valores predefinidos

```tsx copy
<InputGallery defaultValue={[{ fileData: "url1", text: "Image 1" }]} onChange={(items) => { console.log(items); }} />
```

- Galería con botón personalizado

```tsx copy
<InputGallery textBtn="Upload New Image" onChange={(items) => { console.log(items); }} />
```
