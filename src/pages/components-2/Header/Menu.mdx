...
<Iframe src="https://fenextjs-component-storybook.vercel.app/iframe.html?args=&id=component-menu--index&viewMode=story" />
...
```tsx copy
<Menu items={[{ label: 'Home', link: '/' }, { label: 'About', link: '/about' }]} />
```

- Menú con ícono personalizado

```tsx copy
<Menu iconArrow={<CustomIcon />} items={[{ label: 'Home', link: '/' }, { label: 'Services', link: '/services' }]} />
```

- Menú con submenús desplegables

```tsx copy
<Menu defaultShowSubMenu={true} items={[{ label: 'Home', link: '/', defaultActive: true }, { label: 'Projects', link: '/projects' }]} />
```
